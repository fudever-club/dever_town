...
class AudioManager {
  ...
  // ==========================================
  // MINIGAMES 2.0 PROCEDURAL SOUND SYNTHESIZER
  // ==========================================

  /** Sút bóng / Cú đá uy lực */
  playKick(power = 1.0) {
    if (this.isMuted || !this.sfxEnabled) return;
    const ctx = this.getAudioContext();
    if (!ctx) return;
    try {
      ...
      gain.gain.setValueAtTime(this.masterVolume * 0.4 * Math.min(1.2, power), now);
  ...
  /** Bóng đập xà ngang / cột dọc kim loại */
  playPostClang() {
    if (this.isMuted || !this.sfxEnabled) return;
    const ctx = this.getAudioContext();
    if (!ctx) return;
    try {
      ...
      [880, 1760, 2640].forEach((freq, i) => {
        ...
        osc.type = 'sine';
        osc.frequency.setValueAtTime(freq, now);
        gain.gain.setValueAtTime((this.masterVolume * 0.25) / (i + 1), now);
  ...
  playOnFire() {
    ...
    try {
      ...
      [330, 440, 550, 660, 880].forEach((freq, i) => {
        ...
        osc.type = 'sawtooth';
        osc.frequency.setValueAtTime(freq, now + i * 0.04);
        gain.gain.setValueAtTime(this.masterVolume * 0.18, now + i * 0.04);
  ...
  /** Đập bóng chuyền uy lực Spike Smash */
  playSpikeSmash() {
    if (this.isMuted || !this.sfxEnabled) return;
    const ctx = this.getAudioContext();
    if (!ctx) return;
    try {
      ...
      gain.gain.setValueAtTime(this.masterVolume * 0.42, now);
  ...
  /** Barista: Tiếng đá rơi lách cách vào ly */
  playIceClink() {
    if (this.isMuted || !this.sfxEnabled) return;
    const ctx = this.getAudioContext();
    if (!ctx) return;
    try {
      ...
      [1600, 2400].forEach((freq, i) => {
        ...
        const gain = ctx.createGain();
        osc.type = 'sine';
        osc.frequency.setValueAtTime(freq + Math.random() * 200, now + i * 0.03);
        gain.gain.setValueAtTime(this.masterVolume * 0.22, now + i * 0.03);
        gain.gain.exponentialRampToValueAtTime(0.001, now + i * 0.03 + 0.07);
        osc.connect(gain);
        gain.connect(ctx.destination);
  ...
  /** Barista: Tiếng rót chất lỏng róc rách */
  playLiquidPour() {
    if (this.isMuted || !this.sfxEnabled) return;
    const ctx = this.getAudioContext();
    if (!ctx) return;
    try {
      ...
      const gain = ctx.createGain();
      osc.type = 'sine';
      osc.frequency.setValueAtTime(500 + Math.random() * 120, now);
      osc.frequency.linearRampToValueAtTime(650 + Math.random() * 150, now + 0.06);
      gain.gain.setValueAtTime(this.masterVolume * 0.14, now);
  ...
  /** Tiếng tời ròng rọc quay xích sắt */
  playWinchCrank() {
    if (this.isMuted || !this.sfxEnabled) return;
    const ctx = this.getAudioContext();
    if (!ctx) return;
  ...
  /** Sokoban: Tiếng trượt hòm gỗ/kim loại */
  playSokobanSlide() {
    if (this.isMuted || !this.sfxEnabled) return;
    const ctx = this.getAudioContext();
    if (!ctx) return;
  ...
  /** Sokoban: Âm thanh hoàn tác Undo */
  playUndo() {
    if (this.isMuted || !this.sfxEnabled) return;
    const ctx = this.getAudioContext();
    if (!ctx) return;
